...
def get_germplasm_data(base_url):
    ...
    while page < total_pages:
        ...
        url = f"{base_url}traits?page={page}&pageSize=5000"
        print(f'URL: {url}')
        try:
...
server_info = {'T3/Wheat': {'server-title': 'T3/Wheat', 'api-urls': ['https://wheat.triticeaetoolbox.org/brapi/v2/'], 'auth-required': False, 'server_status': True}, 'T3/Barley': {'server-title': 'T3/Barley', 'api-urls': 
['https://barley.triticeaetoolbox.org/brapi/v2/'], 'auth-required': False, 'server_status': True}
}
def read_all_data():
    for server in server_info:
        server_name = server_info.get(server, {}).get('server-title')
        read_data(server_name+"_traits")
