...
def clean_data(data):
    """
    Recursively remove attributes with values that are None, Null, NA, 'NA/NA', empty strings,
    empty lists, empty dictionaries, or zero.
    """
    na_values = [None, 'unknown', 'None', 'null', 'NA', 'NA/NA', '', ' ', [], {}, 0]
    
    if isinstance(data, list):
        cleaned_list = [clean_data(item) for item in data if item not in na_values]
        return [item for item in cleaned_list if item not in na_values and item]
    elif isinstance(data, dict):
        cleaned_dict = {key: clean_data(value) for key, value in data.items() if value not in na_values}
        return {key: value for key, value in cleaned_dict.items() if value not in na_values and value}
    else:
        return data
